...
1. **Mixed-intent only.** Use the conductor ONLY when a request needs BOTH software AND business/growth
   deliverables. Build-only → hand to `agentic-builder` and stop. Grow-only → hand to `intelli-agent`
...
3. **Bridge with a handoff brief.** After BUILD, synthesize `HANDOFF.json` (`references/handoff-contract.md`)
   and feed it to GROW so intelli-agent does NOT re-interview — it grows the REAL product.
...
- `needs_build && !needs_grow` → invoke the `agentic-builder` skill with the whole request. Stop.
- `needs_grow && !needs_build` → invoke the `intelli-agent` skill with the whole request. Stop.
...
## STAGE 2 — BUILD (invoke agentic-builder)

Set `suite-state.phase = "build"`. Tell the user: "Phase 1/2 — BUILD. Handing the software scope to
agentic-builder; its dashboard opens on :4317."

Invoke the **agentic-builder** skill (Skill tool) with `build_brief`. Let it run its full pipeline
...
When agentic-builder finishes (its Phase 9), capture where it wrote outputs and state.

## STAGE 3 — Handoff (synthesize the product brief)

Set `phase = "handoff"`. Build `HANDOFF.json` per `references/handoff-contract.md`.

agentic-builder does not yet emit `HANDOFF.json` natively, so the conductor SYNTHESIZES it from the
...
`.agentic-builder/memory.json` (decisions). (Follow-up: have agentic-builder write `HANDOFF.json` at
Phase 9 directly — then this stage just reads it.)

Write `HANDOFF.json` to the top level AND copy it into `grow/plan/docs/HANDOFF.json` so intelli-agent's
bring-your-own-docs gate picks it up as the product brief.

## STAGE 4 — GROW (invoke intelli-agent)

Set `phase = "grow"`. Tell the user: "Phase 2/2 — GROW. Handing the growth scope to intelli-agent with
the product brief; its dashboard opens on :4318."

Invoke the **intelli-agent** skill (Skill tool) with `grow_brief`, working in `grow/`. Because
`grow/plan/docs/HANDOFF.json` is present, intelli-agent's docs gate uses it as the product context and
...
BUILD uses agentic-builder's board (:4317); GROW uses intelli-agent's (:4318). The conductor announces
